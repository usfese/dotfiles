function f
    ...
    # 最大库存上限，即待展示区（调小减少缓存占用）
    set -l MAX_CACHE_LIMIT 30
    # 库存少于多少张时开始补货（调小降低后台负载）
    set -l MIN_TRIGGER_LIMIT 15

    # used 目录最大存放数量（调小以节省空间）
    # 超过此数量将按照时间顺序删除最旧的文件
    set -l MAX_USED_LIMIT 20
    ...
    function get_random_url
        echo "https://www.loliapi.com/acg/pe/"
    end
    ...
    function download_one_image -V CACHE_DIR
        ...
        if string match -qr "^http" -- "$URL"
            ...
            curl -s -L --connect-timeout 5 --max-time 30 -o "$TARGET_PATH" "$URL"
    ...
    function background_job -V CACHE_DIR -V LOCK_FILE -V MIN_TRIGGER_LIMIT -V DOWNLOAD_BATCH_SIZE -V MAX_CACHE_LIMIT
        ...
        fish -c "
            ...
            if test \$FINAL_COUNT -gt $MAX_CACHE_LIMIT
                set DELETE_COUNT (math \$FINAL_COUNT - $MAX_CACHE_LIMIT)
                find \$CACHE_DIR -maxdepth 1 -name '*.jpg' -printf '%T@ %p\0' | sort -nz | head -z -n \$DELETE_COUNT | while read -lz item
                    set file_path (string split -m1 ' ' -- \$item)[2]
                    if test -n "\$file_path"
                        rm -f "\$file_path"
                    end
                end
    ...
    if test -n "$SELECTED_IMG"; and test -f "$SELECTED_IMG"
        ...
        # === 逻辑: 检查 used 目录并清理旧图 ===
        set -l used_count (find "$USED_DIR" -maxdepth 1 -name '*.jpg' 2>/dev/null | wc -l)

        if test "$used_count" -gt "$MAX_USED_LIMIT"
            set -l to_remove (math "$used_count" - "$MAX_USED_LIMIT")
            find "$USED_DIR" -maxdepth 1 -name '*.jpg' -printf '%T@ %p\0' | sort -nz | head -z -n "$to_remove" | while read -lz item
                set -l file_path (string split -m1 ' ' -- "$item")[2]
                if test -n "$file_path"
                    rm -f "$file_path"
                end
            end
